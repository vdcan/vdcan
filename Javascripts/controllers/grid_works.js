...
app.controller('MainCtrl', function ($scope, $state, $stateParams, $rootScope) {
    ...
    $scope.insert = function () {  
        $rootScope.$broadcast("GridDemoinsert", $scope.row);
    };
    $scope.delete = function (id) {
        $rootScope.$broadcast("GridDemodelete", $scope.row.id, $scope.row.id);
    };
    $scope.update = function () {
        $rootScope.$broadcast("GridDemoupdate", $scope.row);
    }; 
...
app.controller('GridDemoCtrl', [
    '$scope', '$rootScope', '$http', '$modal', function ($scope, $rootScope, $http, $modal) { 
        ...
        $rootScope.$on("GridDemoupdate", function (event,row) {
            updateData (row)
        });
        $rootScope.$on("GridDemoinsert", function (event, row) { 
            insertData(row);
        });
        $rootScope.$on("GridDemodelete", function (event, id, text) {
            deleteIt(id, text)
        });
