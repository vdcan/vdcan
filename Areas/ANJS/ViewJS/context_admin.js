...
app.controller('contextCtrl', [
    ...
    $scope.getDDL = function (param) {
       // console.log(param);
        ...
        $scope.cancelEdit = function (row) {
            ...
            if ($scope.editType == "i") {
                $scope.contextridOptions.data.splice(0, 1);
            }else {
  ...
  cellTemplate:"<div  class='ui-grid-cell-contents'>{{grid.appScope.TranslateToText('1=Active 0=Inactive',row.entity.active_flag)}}</div>"  },
  ...
  cellTemplate:"<div  class='ui-grid-cell-contents'>{{grid.appScope.TranslateToText(grid.appScope.DDLData['context_type'],row.entity.type)}}</div>"  },
        ...
        $scope.getPage = function () {
            $http.get(km.model.urls["context_pager"] + "&page=" + paginationOptions.pageNumber
                ...
                paginationOptions.order + "&type=0"+  "&_t="+com.settings.timestamp()).success(function (result) {
