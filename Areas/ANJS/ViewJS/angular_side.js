...
km.init = function () {
 ...
 //   km.template.init();
         if (typeof Extern_Init != "undefined")	
     Extern_Init();
       ...
       var options = $(this).combobox('options')
        ...
        if (u!= undefined) { 
         ...
         if (loader != undefined) { 
          ...
          var sloader = $(this).attr("sloader");
         if (sloader != undefined) {
             options.loader = function (param, success, error) {
                 var q = param.q || "";
                 if (q.length <= 0) { 
                     return false;
                 }
                 var jsonParam = { loader: sloader, value: q }
                 com.ajax({
                     url: km.model.urls["sloader"], data: jsonParam, success: function (result) {
                         success(result);
                     }
                 });
             };
        ...
        var ddl = $(this).attr("ddl");
         if (ddl != undefined) { 
             options.url = km.model.urls["ddler"]+"&ddl="+ddl;

             $(this).combobox(options);
         }


         var sddl = $(this).attr("sddl");
         if (sddl != undefined) {
             options.url = km.model.urls["sddler"] + "&ddl=" + sddl;
